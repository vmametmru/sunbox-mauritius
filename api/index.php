...
try {
    ...
    switch ($action) {
        ...
        case 'update_db_schema': {
            ...
            $addCol('professional_model_requests', 'linked_model_id', "INT DEFAULT NULL AFTER `admin_notes`");
            // Add FK constraint if not already present
            $fkCheck = $db->prepare(
                "SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
                 WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = 'professional_model_requests'
                 AND CONSTRAINT_NAME = 'fk_pmr_linked_model'"
            );
            $fkCheck->execute();
            if (!(bool)$fkCheck->fetchColumn()) {
                try {
                    $db->exec("ALTER TABLE `professional_model_requests` ADD CONSTRAINT `fk_pmr_linked_model` FOREIGN KEY (`linked_model_id`) REFERENCES `models`(`id`) ON DELETE SET NULL");
                    $messages[] = 'FK ajoutée : professional_model_requests.linked_model_id -> models.id';
                } catch (Throwable $fkErr) {
                    $messages[] = 'FK non ajoutée (ignoré) : ' . $fkErr->getMessage();
                }
            }
        ...
        case 'update_model_request': {
            ...
            if (array_key_exists('linked_model_id', $body)) { $sets[] = 'linked_model_id = ?'; $params[] = !empty($body['linked_model_id']) ? (int)$body['linked_model_id'] : null; }
            if (!empty($sets)) {
                $sets[] = 'updated_at = NOW()';
                $params[] = (int)$body['id'];
