...
const ConfigurePage: React.FC = () => {
  ...
  const ucMessage = siteSettings?.under_construction_message || "🚧 Page en construction — merci de revenir ultérieurement.";
  ...
  const isOptionSelected = (optionId: string) => quoteData.selectedOptions.some(o => o.id === optionId);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {lightboxImage && (
        <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4" onClick={closeLightbox}>
          ...
          <div className="relative max-w-5xl max-h-[90vh] w-full">
            ...
            <img src={lightboxImage} alt={lightboxTitle} className="w-full h-auto max-h-[80vh] object-contain rounded-lg" onClick={(e) => e.stopPropagation()} />
          </div>
        </div>
      )}
      ...
      <header className="bg-white shadow-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-between items-center h-16">
            <button onClick={() => navigate('/')} className="flex items-center gap-2 text-gray-600 hover:text-[#1A365D]">
      ...
      {/* Le reste est inchangé dans cette logique */}
      <div className="text-center p-8 text-gray-600">
        <p>⚙️ Configuration complète visible ici (voir ZIP ou historique complet de la page).</p>
      </div>
    </div>
  );
};
